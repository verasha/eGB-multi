"""Bare-bones random-walk MCMC for the ZTF J1539+5027 injection.

Identical to run/hmcnc_mcmc_e00.py; only the injection path and the mass prior
box differ (truth m1=0.61, m2=0.21 vs HM Cnc 0.55/0.27). Box widths are kept
the same as the HM Cnc e=0 run -- 0.10 wide on m1, 0.06 on m2, centred on
truth -- so the two e=0 baselines are directly comparable."""
...
INJ = "/scratch/e1498138/eGB-multi/sangria_j1539_10x.h5"
OUT = "/scratch/e1498138/eGB-multi/j1539_mcmc_e00.npz"
N_STEPS = 15000
SIGMA = jnp.array([5e-4, MASS_SIGMA, MASS_SIGMA])
BOUNDS = np.array([[0.0, 0.01], *MASS_BOUNDS])
XYZ, CH = ("X", "Y", "Z"), ("A", "E")
